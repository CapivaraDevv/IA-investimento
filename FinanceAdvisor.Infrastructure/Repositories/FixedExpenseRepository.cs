...
{
    public Task<FixedExpense?> GetByIdAsync(Guid id, CancellationToken ct = default) =>
        db.FixedExpenses.FirstOrDefaultAsync(x => x.Id == id, ct);
    ...
    public async Task UpdateAsync(FixedExpense expense, CancellationToken ct = default)
    {
        db.FixedExpenses.Update(expense);
        await db.SaveChangesAsync(ct);
    }
